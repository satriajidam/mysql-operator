	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.schedule != nil {
				...
				mysqlopclient.PrependReactor("update", "backupschedules", func(action core.Action) (bool, runtime.Object, error) {
			...
			if upd := test.expectedSchedulePhaseUpdate; upd != nil {
				action := core.NewUpdateAction(
					v1alpha1.SchemeGroupVersion.WithResource("backupschedules"),
					upd.Namespace,
					upd)
